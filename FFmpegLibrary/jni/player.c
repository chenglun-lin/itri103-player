...
enum WaitFuncRet player_wait_for_frame(struct Player *player, int64_t stream_time,
	...
	while (1) {
		...
		//player->start_time = stream_time;

		int64_t current_video_time = player_get_current_video_time(player);

		int64_t current_time = av_gettime() - player->start_time + player->video_start_time;//;
		...
		int64_t sleep_time = stream_time - current_time;
...
int player_decode_video(struct DecoderData * decoder_data, JNIEnv * env,
	...
	player->width = ctx->width;
	player->height = ctx->height;
	//(*env)->CallVoidMethod(env, player->thiz,
	//						player->player_prepare_frame_method,
	//						ctx->width,
	//						ctx->height);
...
int player_alloc_queues(struct State *state) {
	...
	int stream_no;
	for (stream_no = 0; stream_no < capture_streams_no; ++stream_no) {
		player->packets[stream_no] = queue_init_with_custom_lock(150,
...
Java_com_appunite_ffmpeg_FFmpegPlayer_getVideoWidth(JNIEnv *env, jobject thiz) {
	struct Player * player = player_get_player_field(env, thiz);
	return player->width;
}
JNIEXPORT int JNICALL
Java_com_appunite_ffmpeg_FFmpegPlayer_getVideoHeight(JNIEnv *env, jobject thiz) {
	struct Player * player = player_get_player_field(env, thiz);
